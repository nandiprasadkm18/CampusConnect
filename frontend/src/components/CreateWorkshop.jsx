...
import { Type, AlignLeft, Calendar, MapPin, GraduationCap, BookOpen, CheckCircle, PlusCircle, Users, Image } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!userInfo || !userInfo.token) {
        setError('Authorization required to create workshops.');
        return;
      }
      const config = { 
        headers: { 
      ...
      setSuccess('Workshop session has been successfully added to the system.');
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 font-sans bg-[#f8fafc]/50 min-h-screen">
      ...
      >
        ...
        <div className="bg-[#0f172a] pt-12 pb-10 px-10 text-center relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-500 via-teal-500 to-indigo-500 opacity-50" />
          <div className="inline-flex p-4 rounded-full bg-white/5 mb-6 ring-1 ring-white/10 shadow-inner">
            <BookOpen className="w-10 h-10 text-white" />
          </div>
          <h2 className="text-4xl font-extrabold text-white font-serif mb-3 tracking-tight">Host Academic Workshop</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-10">
            ...
            <div className="space-y-4">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.15em] flex items-center gap-2.5">
                <Type className="w-3.5 h-3.5" /> Workshop Title
              </label>
              <input 
            ...
            <div className="space-y-4">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.15em] flex items-center gap-2.5">
                <GraduationCap className="w-3.5 h-3.5" /> Target Branch
              </label>
            ...
            <div className="space-y-4">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.15em] flex items-center gap-2.5">
                <PlusCircle className="w-3.5 h-3.5 text-indigo-500" /> Workshop Category
              </label>
              <div className="relative">
                <select 
                  className="w-full px-6 py-4.5 bg-[#f8fafc] border border-slate-200 rounded-2xl text-slate-900 font-bold focus:bg-white focus:ring-4 focus:ring-indigo-50 focus:border-indigo-200 transition-all outline-none appearance-none cursor-pointer text-base"
            ...
            <div className="space-y-4">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.15em] flex items-center gap-2.5">
                  <Users className="w-3.5 h-3.5 text-indigo-600" /> Max Squad Size
                </label>
                <div className="flex items-center gap-4">
                  <input 
                    className="w-32 px-6 py-4.5 bg-[#f8fafc] border border-slate-200 rounded-2xl text-slate-900 font-bold focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all outline-none text-base"
                    type="number" 
                    value={maxGroupSize} 
                    ...
                    max="50"
                  />
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Students per registration</span>
                </div>
            </div>
            ...
            <div className="space-y-4 md:col-span-2">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.15em] flex items-center gap-2.5">
                <AlignLeft className="w-3.5 h-3.5" /> Workshop Scope
              </label>
              <textarea 
                ...
                placeholder="Outline the learning objectives and prerequisites..."
                required 
              />
            </div>
            ...
            <div className="space-y-4">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.15em] flex items-center gap-2.5">
                <MapPin className="w-3.5 h-3.5" /> Venue Details
              </label>
              <input 
                ...
                placeholder="e.g., Computer Lab 302"
                required 
              />
            </div>
            ...
            <div className="space-y-4 md:col-span-2 group">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.15em] flex items-center gap-2.5">
                <Image className="w-3.5 h-3.5" /> Workshop Banner (Optional)
              </label>
            ...
            <div className="space-y-6 md:col-span-2 p-10 bg-[#f8fafc] border border-slate-100 rounded-[2.5rem] shadow-inner">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-xs font-black text-slate-900 uppercase tracking-widest">Paid Workshop Required?</p>
